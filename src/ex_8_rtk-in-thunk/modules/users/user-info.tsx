...
export function UserInfo() {
    ...
    // const {isLoading: isLoadingDelete} = usersApi.endpoints.deleteUser.select(id ?? skipToken);
    const isLoadingDelete = useAppSelector((state) =>  /* (получение значения выше без деструктуризации - нужно чтобы получить определенное значение true/false, а не обьект, как при деструктуризации - чтобы избавиться от лишних рендеров) */        
    ...
    const handleDeleteButtonClick = async () => {
        ...
        // await deleteUser(id);
        // navigate("..", {relative: "path"});
        await dispatch(deleteUser(id)); /* (при удалении используем функцию из thunk) */
     }
